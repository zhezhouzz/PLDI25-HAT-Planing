...
\begin{example}[Stack Library] Consider an effectful stack library
...
\begin{example}[Read Atomicity]
  ...
  \begin{align*}
    &\text{\textcolor{gray}{Client 1:}}  & \zevent{writeReq}{4,\tau_1};\zevent{writeRsp}{4,\tau_1};&
    \\& \text{\textcolor{gray}{Client 2:}} \quad \zevent{readReq}{\tau_2};\zevent{readRsp}{3,\tau_2};\zevent{readReq}{\tau_2};& &\zevent{readRsp}{4,\tau_2}
  \end{align*}}
  Here, \textcolor{gray}{Client 2} issues two consecutive read
  operations.  A $\eff{readReq}$ event initiates a request to the
  database with transaction id $\tau_2$; the response from the
  ...
  id $\tau_1$, that occurs after the request.  The following
  trace specification on
  $\eff{readReq}$ events admits executions that satisfy read atomicity:
   \begin{align*}
    \eff{readReq} :~ \rg{\Code{LAST}(\zevent{readRsp}{x,\tau})}{\eff{readReq}}{\Code{FIRST}(\zevent{readRsp}{x,\tau})}
  \end{align*}
...
\ZZ{Shall we use another character than $\tau$?}
...
%%       % \eff{app} :~ & \rg{\allA}{\msgC{app}}{\allA  \msgAGray{getType}{\Code{Arr}(t_1, t_2)}  \msgC{appL}  \allA  \msgAGray{getType}{t_1}  \msgC{appR}  \msgAGray{putType}{t_2}  \allA} \\
 & \eff{abs} :~   [\Code{LAST}(\msgAGray{depth}{d})]
                 [\msgA{abs}{t_1}]
                 [\msgAGray{depth}{d+1}\allA  \msgA{var}{0}\allA\Code{LAST}(\msgAGray{ty}{t_2})\msgC{endAbs}\msgAGray{depth}{d}]
...
abstraction references its bound variable at least once, thereby
ruling out trivial terms like $e_1$.

\ZZ{$\eff{var}$ doesn't have to be $0$}
...
traces, based on the $\eff{abs}$ specification given in
\autoref{fig:spec}.  For readability, we assume the trace of the
...
\\&\alpha_1 \doteq\absTrace{\textcolor{orange}{\Code{Int}}}{1}{\boxed{\alpha_2}}{0}{\ItoI}
\\&\alpha_2 \doteq\absTrace{\textcolor{DeepGreen}{\ItoI}}{2}{\boxed{\alpha_3; \eff{var}(\textcolor{orange}{1})}}{1}{\Arr{\ItoI}{\Code{Int}}}
\\&\alpha_3 \doteq\absTrace{\textcolor{blue}{\Code{Int}}}{2}{\boxed{\eff{var}\; \textcolor{blue}{[0]}}}{1}{\ItoI}
\end{align*}}\noindent
...
$\alpha_3$ has type $\Code{Int}$ (blue) at depth $2$.  Notably, the bound
variable $[1]$ in $\alpha_2$ is assigned type $\ItoI$ (orange) because
its binding occurs at depth $1$, where the type annotation for $\ItoI$
