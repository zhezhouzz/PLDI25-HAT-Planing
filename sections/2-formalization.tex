  ...
  The following derivation illustrates the execution of a single iteration of the program from Example~\ref{ex:read-atomicity}, evaluated under the auxiliary semantics for $\eff{writeReq}$ and $\eff{readReq}$ specified in Example~\ref{ex:aux-semantics}, which enforce read atomicity. For this iteration, we assign transaction identifiers $\Code{tid1}=1$ and $\Code{tid2}=2$, and set $x=7$:
    {\footnotesize
      \begin{flalign*}
        ...
        \\[-0.4em]
        &\xhookrightarrow{[\textcolor{orange}{\zevent{readReq}{2}]}}{(\{\zevent{readReq}{2}\}, \eff{writeReq}\;1\;\Code{int\_gen()};\zobs{\_}{writeRsp}{\textit{line 6 of code in Example~\ref{ex:read-atomicity}}})}  \tag{\textsc{StGen}} &&
        \\[-0.4em]
        ...
        \\[-0.4em]
        &\xhookrightarrow{[\zevent{writeRsp}{1,8}]}{(\{\zevent{readReq}{2}\}, \zobs{\_, y}{readRsp}{\sassert{y == 7}; () \oplus \Code{transaction\_gen\;1\;2}})}  \tag{\textsc{StObs}} &&
        \\[-0.4em]
        &\xhookrightarrow{[\zevent{readRsp}{2,7}]}{(\emptyset, \sassert{7 == 7}; () \oplus \Code{transaction\_gen\;1\;2})}  \tag{\textsc{StObs}} &&
      \end{flalign*}
    }\noindent
  In the final step, the program executes $\mykw{obs}\;\eff{readRsp}$, producing a read value $y=7$, which matches the value of the most recent $\eff{writeRsp}$ (blue) at the point where the $\eff{readReq}$ event (orange) occurred. Thus, read atomicity is preserved and execution can continue to generate further transactions.
\end{example}
...
For example, the constant value $1$ can have overapproximate type $\ort{\Int}{\vnu = 1}$ and $\ort{\Int}{\top}$; however, it cannot have underapproximate type $\urt{\Int}{\top}$ which can only has inhabitant such as $\Code{int\_gen ()}$.
...
When the fields of an event are clear from context, we omit its parameters
  $\overline{x}$, e.g., $\msg{writeReq}{v > 0}$ means
  $\msgB{writeReq}{\I{tid}\;v}{v > 0}$. We omit the quantifiers with equality constraints, e.g., $\msg{writeReq}{\I{tid} = 1 \land v = 3}$, as $\msgA{writeReq}{1\;3}$; we also omit the top quantifiers in symbolic events and types.
...
\begin{example}[Generator Typing]
  ...
  {\footnotesize\begin{align*}
    ...
    {readReq}{i_2}\msg{writeReq}{\I{tid} = i_1}\msg{writeRsp}{\I{tid} = i_1}\msgA{readRsp}{i_2\;x}}_{F_2}}
  \end{align*}}\noindent
  Here, $i_1$ and $i_2$ are function parameters (overapproximated), $x$ is a random integer (underapproximated), and the recursive reference $\Code{transaction\_gen}$ is assigned the empty type $\uhat{\ldots}{\emptyset}$. This right-hand side type precisely underapproximates the function body, ensuring the read response returns the value written by $x$.
