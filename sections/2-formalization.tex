...
\autoref{fig:term-syntax}; programs are written in A-normal form and
...
detail. Similar to other calculi for trace-based type systems, the
reduction relation defining this semantics,
...
Similar to other recent trace-based type systems~\cite{ZYDJ24},
$\DSL{}$ uses \emph{Symbolic Finite Automata} (SFAs)~\cite{Vea13,
...
Besides standard type judgement on pure values $\Gamma \vdash v : t$,
we introduce a typing judgment $\Gamma\vdash e: \uhat{H}{x{:}t}{F}$
enables a Hoare-triple-style ($\effLR{H}\;e\;\effLR{F}$) reasoning in
type system. The type context, $\Gamma$ maps from variables to pure
refinement types (i.e., $t$). As in other trace-based refinement type
systems, contexts are not allowed to contain \tyName{}s--- doing so
...
relation $\Gamma \wellfoundedvdash \tau$ which mostly ensures, e.g.,
that all qualifiers appearing in a type $\tau$ are closed under the
current typing context $\Gamma$; an inclusion relation on SREs
$\Gamma \vdash A \subseteq A$; and a mostly-standard semantic
subtyping relation.  \autoref{fig:selected-typing-rules} presents the
key subtyping rule for \tyName{}s (\textsc{SubHF}), which establishes
subtyping by checking inclusion between their history and future
regexes under the current type context $\Gamma$. As in
Underapproximate Logic with the reverse principle of consequence, our
subtyping relation is covariant in the history regex and contravariant
in the future regex, in contrast to prior overapproximated trace-based
refinement type systems~\cite{ZYDJ24}.

The pure refinement types at positive position is underapproximated
(i.e., $\urt{b}{\phi}$) and at negative position (function's parameter
type) is overapproximated (i.e., $\ort{b}{\phi}$).  We use
$\widehat{t}$ to denote the flip a base type from underapproximate to
overapproximate and vice versa; the function type does not change.
Verification conditions generated by our type-checker can be encoded
...
corresponding well-formedness judgments from their premises.  In
underapproximation reasoning, we cannot apply the subsumption rule
(\textsc{TSub}) to arbitrary type contexts. For example, consider the
read atomicity property shown in Example~\ref{ex:read-atomicity},
which cannot be detected by the following code that only write the
same value:
...
\noindent However, the subsumption rule may allow the random integer
$x$ to be specialized to different values (i.e., $7$ and $8$) at
different calling points, which violates the call-by-value semantics.
Following prior underapproximate refinement type
systems~\cite{CoverageType}, the subsumption rule (\textsc{TSub}) is
restricted to empty type contexts ($\emptyset$) to ensure the
soundness of reachability reasoning.  In contrast, the equivalence
rule (\textsc{TEq}) permits type transformation to an semantically
equivalent type under arbitrary contexts.
...
The rules for performing events, \textsc{TGen} and \textsc{TObs}, both
extract the operator type $\uhat{H}{t}{\msg{op}{\overline{x}}\seqA F}$
from operator context $\Delta$; and require that it aligns with the
\tyName{} of the expression that the operation is being performed in:%
...
\begin{example}[Generator Typing]
  ...
  \autoref{ex:read-atomicity}, demonstrating that the first iteration
  satisfies the assertion. By the \textsc{FixBase}, \textsc{TFixBase},
  and \textsc{TFun} rules, we need to type check the body of the
  recursive function against the type: %
  \vspace{-.4cm} \par\nobreak {\footnotesize %
    \begin{align*}
