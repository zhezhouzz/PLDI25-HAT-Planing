...
globally\footnote{Specifically, the formula \(\allTL\), \(\epsTL\), and \(\empTL\) represent the set of all possible traces, the singleton set containing the empty trace ($\{[]\}$), and the empty set of traces, respectively, analogous to \( .^* \), \(\epsilon\), and \(\emptyset\) in regular expressions.} ($\globalA$), and importantly, the singleton (last) modality
...
\begin{example}[Controller Typing] We provide an informal typing
  ...
  {\footnotesize
    \begin{align*}
      &\underline{\Delta(\eff{writeReq}) = \langle \Code{x}\hasnty{int}
