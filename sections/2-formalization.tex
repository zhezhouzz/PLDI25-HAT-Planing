...
{\footnotesize
  \begin{alignat*}{2}
    ...
    \\\text{\textbf{Effectful Operations}}& \quad & \eff{op} ::= \quad & \eff{abs} ~|~ \eff{endAbs} ~|~ \eff{putTy} ~|~ \eff{ayncGetTy} ~|~ \eff{getTy} ~|~ ...
...
The auxiliary semantics of the $\eff{abs}$ and $\eff{getTy}$ operators can be defined as follows:
% (we omit operators that do not produce new capabilities, such as $\eff{endAbs}$):

{\footnotesize
  \mprooftr{25mm}{}{OpAbs}{
  $\alpha \vDash \eff{abs} \Downarrow \{\eff{endAbs}\}$
  }
  \quad
  \mprooftr{58mm}{$\effGray{putTy}\not\in \alpha_2$}{OpGetTy}{
  $\alpha_1\listconcat\zeventGray{putTy}{t}\listconcat\alpha_2\vDash \effGray{ayncGetTy} \Downarrow \{\zeventGray{getTy}{t}\}$
  }
  \\[0.1em]
  % \\[0.8em]
  % \mprooftr{30mm}{}{OpEndAbs}{
  % $\alpha \vDash \eff{endAbs} \Downarrow \emptyset$
  % }
  % \quad
  % \mprooftr{30mm}{}{OpPutTy}{
  % $\alpha\vDash \zevent{putTy}{t} \Downarrow \emptyset$
  % }
  % \quad
  % \mprooftr{30mm}{}{OpGetTy2}{
  % $\alpha\vDash \zevent{getTy}{t} \Downarrow \emptyset$
  % }
  }\noindent

\noindent The $\eff{abs}$ operator simply adds one event $\eff{endAbs}$ to the capability. Our language simulates the synchronized operation $\zeventGray{getTy}{v}$ as a generative event $\effGray{ayncGetTy}$ followed by an adjacent observable event $\zeventGray{getTy}{v}$ which always returns the last ``$\effGray{putTy}$''.
\end{example}
...
%     &\denot{\rg{H}{A}{F}} \doteq \{e ~|~ \emptyset \basicvdash e : \Code{unit} \land \forall \alpha_h \in\denot{H}. \forall \alpha \in\denot{A}. \forall \beta_1\;\beta_2.
%     \\[-0.4em] & \ \ \underbrace{\exists e_h. \msteptr{[]}{(\emptyset, e_h)}{\alpha_h}{(\beta_1, ())}}_\texttt{realizable histroy trace} \land \underbrace{\msteptr{\alpha_h}{(\beta_1, e)}{\alpha}{(\beta_2, ())}}_\texttt{trace produced by $e$} \impl \forall \alpha_f. \alpha_f \in\denot{F} \impl \underbrace{\exists e_f.\msteptr{\alpha_h \listconcat \alpha }{(\beta_2, e_f)}{\alpha_f}{(\emptyset, ())}}_\texttt{realizable future trace} \}
%     % \\&\denot{\rgOver{H}{A}{F}} \doteq \{e ~|~ \emptyset \basicvdash e : \Code{unit} \land \forall \alpha_h \in\denot{H}. \forall \alpha \in\denot{A}. \forall \beta_1\;\beta_2.
%     % \\[-0.4em] & \ \ \underbrace{\exists e_h. \msteptr{[]}{(\emptyset, e_h)}{\alpha_h}{(\beta_1, ())}}_\texttt{realizable histroy trace} \land \underbrace{\msteptr{\alpha_h}{(\beta_1, e)}{\alpha}{(\beta_2, ())}}_\texttt{trace produced by $e$} \impl \forall \alpha_f. \underbrace{\exists e_f. \msteptr{\alpha_h \listconcat \alpha }{(\beta_2, e_f)}{\alpha_f}{(\emptyset, ())}}_\texttt{realizable future trace} \impl \alpha_f \in\denot{F} \}
%   \end{align*}}
%   \vspace*{-.15in}
...
types in $\DSL$ are denoted as their inhabitants (i.e., $\denot{t}$ and $\denot{\tau}$).  The capability context is denoted as  observation capabilitys, while the type context $\Gamma$ is denoted as \emph{substitution} $\sigma$ (i.e., $[x_1\mapsto v_1, x_2\mapsto v_2]$) that provides the assignments for binding variables in $\Gamma$. Moreover, the denotation (denoted language) of SREs is the set of traces. Then, regex
...
{\footnotesize\begin{align*}
  ...
  % \\[-0.4em] & \ \ \underbrace{\exists e_h. \msteptr{[]}{(\emptyset, e_h)}{\alpha_h}{(\beta_1, ())}}_\texttt{realizable histroy trace} \land \underbrace{\msteptr{\alpha_h}{(\beta_1, e)}{\alpha}{(\beta_2, ())}}_\texttt{trace produced by $e$} \impl \forall \alpha_f. \underbrace{\exists e_f. \msteptr{\alpha_h \listconcat \alpha }{(\beta_2, e_f)}{\alpha_f}{(\emptyset, ())}}_\texttt{realizable future trace} \impl \alpha_f \in\denot{F} \}
\end{align*}}\noindent
 A term $e$ inhabits $\rg{H}{A}{F}$ iff, for all realizable (i.e., producible by the execution of some program) history traces $\alpha_h$ and current trace $\alpha$ produced by $e$ is accepted by $H$ and $A$, all future traces $\alpha_f$ accepted by $F$ are also realizable.\footnote{The details of these definition can be found in our \techreport{}.} 
 
 Unlike history and current regex whose denotations are overapproximated (typically ensuring safety), the denotation of a prophecy regular expression is underapproximated: the constraint $\alpha_f \in \denot{F}$ appears on the left-hand side of the implication. This underapproximation characterizes traces where, during test generation, $e$ should be explored further, as there exists a continuation $e_f$ that extends the current trace to satisfy the prophecy constraint. For example, in Example~\ref{ex:read-atomicity}, the prefix trace $\zevent{readReq}{\tau_2};\zevent{readRsp}{\tau_2,3};\zevent{readReq}{\tau_2};\ldots$ admits a possible future in which the read atomicity property holds (with a suffix $\,\ldots;\zevent{readRsp}{\tau_2,3}$), although there also exists realizable futures that violate this property.
...
\begin{corollary}[Type Soundness]\label{theorem:sound} Given a operator context \(\Delta\) and a global property \(A\), a generator \(e\) that satisfies \(\emptyset;\Delta;\emptyset \vdash e : \capTau{\emptyset, \rg{\epsilon}{A}{\epsilon}}\) will consistent with both global property and operator context, i.e., $\forall \alpha. \msteptr{[]}{(\emptyset, e)}{\alpha}{(\emptyset, ())} \impl \alpha \in \denot{A} \land \traceDelta{\Delta}{\alpha}$.
\end{corollary}
