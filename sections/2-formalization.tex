...
\begin{alignat*}{2}
    ...
    \\ \text{\textbf{Prophecy Regex Types}}& \quad & \tau  ::= \quad & \uhat{H}{F} ~|~ x{:}b \garr \tau ~|~ \tau \interty \tau
...
computations, and Incorrectness Hoare Automata Types (\tyName{}s), which describe
...
% When a \tyName has no contraint on future computation, we omit its prophecy regex as $\ha{H}{A}$.
...
\paragraph{Ghost variables and ghost events}
...
Additionally, we introduce \emph{ghost events} (displayed in $\effGray{gray}$ to distinguish them from regular effect operators), enabling our \tyName to specify non-regular properties, such as counting.
Both ghost variables and ghost events are not manifested in program terms and are independent of the operational semantics; they serve exclusively as bookkeeping annotations within the type system.
...
Our typing judgment $\Gamma\vdash e: \capTau{\Theta', \tau}$ features three contexts: a type context $\Gamma$,
...
safety. The operator context, $\Delta$, provide the relation between operations, specification of its operator as a \tyName{}, and the operations
...
The typing judgment $\Gamma\vdash e: \capTau{\Theta', \tau}$ asserts that the capability $\Theta$ is sufficient for the term $e$ to consume, and that $e$ will leave the remaining capability $\Theta'$.
The typing judgment for the effectful operator ($\Gamma\vdash \eff{op}: \capTau{\Theta', \tau}$) is similar, but omits the capability context $\Theta$ since it is clear from the message kind.
...
$\Gamma \vdash A_2 \subseteq A_1$  }
{SubU}
{$\Gamma \vdash \uhat{H_1}{A_1} <: \uhat{H_2}{A_2}$}
\quad
\mprooftr{33mm}
{$\Gamma \vdash H_2 \subseteq H_1$ \quad
$\Gamma \vdash A_2 \subseteq A_1$  }
{SubO}
{$\Gamma \vdash \ohat{H_1}{A_1} <: \ohat{H_2}{A_2}$}
\quad
\mprooftr{28mm}
{$\Gamma \vdash H \not\subseteq \emptyset $ }
{SubOU}
{$\Gamma \vdash \ohat{H}{A} <: \uhat{H}{A}$}
\\[.8em] 
\mprooftr{30mm}
...
\mprooftr{40mm}
{
$\Gamma\vdash e_1 : \uhat{H_1}{A_1}$ \quad $\Gamma\vdash e_2 : \uhat{H_2}{A_2}$}
{TChoice}
{$\Gamma\vdash e_1 \oplus e_2 : \uhat{H_1\lorA H_2}{A_1 \lorA A_2}$}
\\[.8em]
\mprooftr{20mm}
{}
{TRet}
...
\mprooftr{61mm}
{
$\Gamma\vdash \eff{op} : \overline{x_i{:}\ort{b_i}{\phi_i}}\sarr \uhat{H}{\msg{op}{\phi} \seqA G}$ \quad
$\forall i. \;\Gamma;\Delta;\emptyset \vdash v_i : \overline{\urt{b_i}{\phi_i}}$ \quad
$\Gamma\vdash e : \uhat{H \seqA \msg{op}{\phi\msubst{x_i}{v_i}} \seqA G}{A}$}
{TGen}
{$\Gamma\vdash \zgen{op}{\overline{v_i}}{e} : \uhat{H}{\msg{op}{\phi\msubst{x_i}{v_i}} \seqA G \seqA A}$}
\\[.8em]
\mprooftr{35mm}{
 $\Gamma, x{:}t_x; \Delta\vdash e : \tau$
}{TFun}{
 $\Gamma\vdash \zlam{x}{\eraserf{t_x}}{e} : x{:}t_x\sarr \tau$}
\quad \
\mprooftr{66mm}{
  $\Gamma\vdash \eff{op} : \overline{x_i{:}\ort{b_i}{\phi_i}}\sarr \uhat{H}{\msg{op}{\phi} \seqA G}$ \quad
  $\Gamma, \overline{x_i{:}\urt{b_i}{\phi_i}}; \Delta\vdash e : \uhat{H \seqA \msgB{op}{\overline{y}}{\phi \land \overline{y = x}} \seqA G}{A}$}
{TObs}
{
  $\Gamma\vdash \zobs{\overline{x}}{op}{e} : \uhat{H}{\msgB{op}{\overline{y}}{\phi} \seqA G \seqA A}$
}
\\[.8em]
\mprooftr{40mm}{
 $\Gamma, f{:}t; \Delta\vdash \zlam{x}{b}{e} : t$
}{TFix}{
 $\Gamma\vdash \zfix{f}{\eraserf{t}}{x}{b}{e} : t$}
\quad
\mprooftr{60mm}{
 $\Gamma\vdash v_1 : y{:}t_y\sarr\uhat{H}{A}$ \quad
 $\Gamma; \Delta\vdash v_2 : t_y$ \quad
...
subtyping relation.
\autoref{fig:selected-typing-rules} presents the key subtyping rule for \tyName{}s (\textsc{SubHAF}), which establishes subtyping by checking inclusion between their constituent regex under the current type context $\Gamma$.
...
The rules for performing events, \textsc{TGen} and \textsc{TObs}, both extract the operator type $\rg{H}{\msg{op}{\phi}}{A \seqA F}$ and capability $\Theta_{\eff{op}}$ from operator context $\Delta$; and require that it aligns with the \tyName{} of the expression that the operation is being performed in:%
...
\vspace{-.4cm} \par\nobreak {\footnotesize %
  \begin{align*}
    \text{\textcolor{gray}{(term's \tyName)}}
    \underbrace{ H
    ...
    \text{\textcolor{gray}{(operator's \tyName)}}
  \end{align*}}\noindent %
...
\begin{example}[Generator Typing] We provide an informal typing
  derivation of the generator in \autoref{fig:stlc-gen} against a \tyName{} that encodes the global property \ref{prop:stlc} as $\rg{\epsilon}{A_\Code{STLC}}{\epsilon}$. The first step of our derivation uses \textsc{TSub} to refine our target type to a \tyName{} that better aligns with the generator code: %
  \vspace{-.4cm} \par\nobreak  {\footnotesize %
    \begin{align*}
   ...
   \vspace{-.4cm} \par\nobreak {\footnotesize
    \begin{align*}
      &\Gamma\vdash \effGray{putDepth} : \langle \emptyset, x\hasnty{int}
        \sarr \rg{\allA}{\msgAGray{putDepth}{x}}{\allA} \rangle <: \langle \emptyset, x{:}\nuot{int}{\vnu = 0}\sarr \rg{\epsilon}{A_1}{A_2A_3} \rangle \\
      &\underline{\makebox[0.8\linewidth][l]{$\Gamma \vdash 0 : \nuot{int}{\vnu = 0} \quad \Gamma\vdash \eff{app};... : \rg{A_1}{A_2 A_3}{\epsilon} \tag{\textsc{TGen}}$}} \\
      &\Gamma\vdash \effGray{putDepth}\;0;\eff{app};... : \rg{\epsilon}{A_1 A_2 A_3}{\epsilon} <: \rg{\epsilon}{A_\Code{STLC}}{\epsilon}
    \end{align*}
  }\noindent
  Since the type of $\effGray{putDepth}$ aligns with the target type, we continue typing the rest of the generator against the \tyName{} $\rg{A_1}{A_2A_3}{\epsilon}$ in the same way.
  \vspace{-.4cm} \par\nobreak {\footnotesize
    \begin{align*}
      &\Gamma\vdash \eff{app} : \langle \{\eff{appL}, \eff{appR}\}, t_1{:}\Code{ty}\sarr t_2{:}\Code{ty}\sarr \rg{\allA}{\msgC{app}}{\allA  \msgAGray{getTy}{\Code{Arr}(t_1, t_2)}  \msgC{appL}  \allA  \msgAGray{getTy}{t_1}  \msgC{appR}  \msgAGray{putTy}{t_2}  \allA}  \rangle \\
      ...
      &\Gamma\vdash \eff{app};... : \rg{A_1}{A_2 A_3}{\epsilon}
    \end{align*}
  }\noindent %
\end{example}
...
\ZZ{Problem: denotation of type context}
...
{\small\begin{align*}
  ...
  \\&\denot{\ohat{H}{A}} \doteq \{e ~|~ \emptyset \basicvdash e : \Code{unit} \land \forall \alpha \in \denot{A}. \forall \alpha_h \in\denot{H}. \exists \beta\;\beta'. \msteptr{\alpha_h}{(\beta, e)}{\alpha}{(\beta',  ())} \}
\end{align*}}\noindent
