...
\parbox{7cm}{\centering$\text{\textbf{Obligation}}\  \beta \in \mathcal{P}(m)$}  &
...
\begin{alignat*}{2}
    ...
    \\& \quad &  \quad & \text{\textcolor{gray}{in Symbolic LTL$_f$ }}  se ~|~ \negA A ~|~ A \landA A ~|~ A \lorA A ~|~ A \seqA A ~|~ \nextA A ~|~ A \untilA A
  ...
  \vspace*{-.1in}
  \begin{flalign*}
    \text{\textbf{Type Alias}}& \quad \msg{op}{\phi} \doteq \msgB{op}{\overline{x}}{\phi} \text{ when the feild name $\overline{x}$ are clear} \quad   \ha{H}{A} \doteq \rg{H}{A}{\epsilon}
  \end{flalign*}
}
\vspace*{-.15in}
...
{\small
\begin{align*} 
  \Delta \doteq \{ (\eff{readReq}, \langle \ldots, \{\eff{readRsp}\}\rangle),%
...
\rg{H}{\msg{op}{\phi}}{A}) \in \Delta$ \quad
...
\mprooftr{81mm}
{
  $(\eff{op}, \Theta', \overline{x_i{:}t_i}\sarr \rg{H}{\msgB{op}{\overline{y}}{\phi}}{A}) \in \Delta$ \quad
...
\ZZ{Why is the future automaton contravariant? Any future trace is allowed; which one is realized depends on the continuation of the program. The PAT can attempt to predict a \emph{subset} of the allowed future traces. As a result, there may exist valid computations that violate the predicted future automaton -- thus it is unsound to say the future automaton is covariant.}
...
{\small\begin{align*}
  ...
  \\&&& \quad \msteptr{[]}{(\emptyset, e_h)}{\alpha_h}{(\beta_1, ())}\land \msteptr{\alpha_h}{(\beta_1, e)}{\alpha}{(\beta_2, ())} \land \msteptr{\alpha_h \listconcat \alpha }{(\beta_2, e_f)}{\alpha_f}{(\emptyset, ())} \impl \alpha \in\denot{A} \}
...
{\footnotesize
  \begin{align*}
    ...
    \msg{op}{\phi\msubst{x_i}{v_i}} \seqA A }_\texttt{current}
    \;\seqA\; \underbrace{ \epsilon }_\texttt{prophecy} \equiv \underbrace{ H
    }_\texttt{history}\;\seqA\;\underbrace{
    \msg{op}{\phi\msubst{x_i}{v_i}} }_\texttt{current} \;\seqA\;
    \underbrace{ A}_\texttt{prophecy}
  \end{align*}}
\noindent %
...
\begin{definition}[Well-formed handler context]
  ...
  The handler specification $\Delta$ is well-formed iff for all operator $\eff{op}$ and its \PAT $\overline{y{:}b} \garr \overline{x{:}t} \sarr \rg{H}{\msg{op}{\phi}}{F}$ and obligation $\overline{\eff{op_i}}$ in $\Delta$ satisfying
{\small\begin{align*}
...
\begin{definition}[Trace consistent with handler context]
  ...
  A trace $\alpha$ is consistent with a handler context $\Delta$ iff for all events $\zevent{op}{\overline{c_i}}$ within $\alpha$ such that $\alpha_h \listconcat [\zevent{op}{\overline{c_i}}] \listconcat \alpha_f = \alpha$, there exists a \PAT $\overline{x{:}t} \sarr \rg{H}{\msg{op}{\phi}}{F}$ and obligation $\Theta$ in $\Delta$ satisfying
{\small\begin{align*}
    \alpha_h \in \denot{H\msubst{x_i}{c_i}} \land \alpha_f \in \denot{F\msubst{x_i}{c_i}} \land \Theta \in \mathbf{GetOp}(\alpha_f) \land \phi\msubst{x_i}{c_i} \land \forall i. c_i \in \denotation{t_i}
\end{align*}}
\end{definition}

\begin{theorem}[Fundamental Theorem]\label{theorem:fundamental}A well-typed term, i.e., \(\Gamma; \Delta; \Theta \vdash e : \ha{H}{A}{F}\), generates traces consistent with the \PAT.\footnote{The proofs of all theorems in this paper are provided in the \techreport{}.}
  {\small\begin{align*}
     &\textcolor{red}{\forall \sigma \in
      \denotation{\Gamma}. \sigma(e) \in
      \denotation{\sigma(\rg{H}{A}{F})}}
  \end{align*}}
  \end{theorem}
  
\begin{theorem}[Type Soundness]\label{theorem:sound} Given a well-formed handler specification \(\Delta\), with ghost variables \(\overline{x{:}b}\) and a violation property \(A\), a controller \(e\) that satisfies \(\overline{x{:\rawnuot{b}{\top}}};\Delta;\emptyset \vdash e : \ha{\epsilon}{A}\) will realize at least one trace consistent with \(A\), i.e., 
   {\small\begin{align*} \textcolor{red}{\forall
      \overline{c{:}b}. \forall \alpha. \msteptr{[]}{(\emptyset,
        e[\overline{x\mapsto c}])}{\alpha}{(\emptyset, ())} \implies \alpha \in
      \denot{A[\overline{x\mapsto c}]} \land \Delta \sim \alpha } 
  \end{align*}}
\end{theorem}