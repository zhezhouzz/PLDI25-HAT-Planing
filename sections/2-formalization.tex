...
We show a program that generates STLC term $\lambda.[0]$.
  {\footnotesize
    \begin{flalign*}
      ...
      \\[-0.4em]
      &\xhookrightarrow{[\zevent{abs}{\Int}]} (\{\eff{endAbs}\}, \eff{var}\;0;\mykw{obs}\;\eff{endAbs})  \xhookrightarrow{[\zevent{var}{0}]} (\{\eff{endAbs}\}, \mykw{obs}\;\eff{endAbs})  \tag{\textsc{StGen}} &&
      \\[-0.4em]
      &\xhookrightarrow{[\eff{endAbs}]} (\emptyset, ()) \tag{\textsc{StObs}} &&
    \end{flalign*}
  }\noindent
...
\begin{corollary}[Type Soundness]\label{theorem:sound} Given a operator context \(\Delta\) and a global property \(A\), a generator \(e\) that satisfies \(\emptyset;\Delta;\emptyset \vdash e : \capTau{\emptyset, \rg{\epsilon}{A}{\epsilon}}\) will consistent with both global property and operator context, i.e., $\forall \alpha. \msteptr{[]}{(\emptyset, e)}{\alpha}{(\emptyset, ())} \implies \alpha \in \denot{A} \land \traceDelta{\Delta}{\alpha}$.
\end{corollary}