...
that is tagged with whether it is observable or generable, and the operations
...
types in $\DSL$ are denoted as their inhabitants (i.e., $\denot{t}$ and $\denot{\tau}$).  The capability context is denoted as message buffers, while the type context $\Gamma$ is denoted as \emph{substitution} $\sigma$ (e.g., $[x\mapsto 3, y\mapsto 4]$ in \autoref{ex:semantics}) that provides the assignments for binding variables in $\Gamma$. Moreover, the denotation (accepting language)
...
\subseteq \denot{\sigma(B)}$.\footnote{The details of these definition can be found in our \techreport{}.}
% {\small\begin{align*}
...
\begin{theorem}[Fundamental Theorem]\label{theorem:fundamental}A well-typed term, i.e., \(\Gamma; \Delta; \Theta \vdash e : \rg{H}{A}{F}\), generates traces consistent with the \PAT and can also terminate with a message buffer denoted by capability $\Theta$.\footnote{The proofs of all theorems in this paper are provided in the \techreport{}.}
{\small\begin{align*}
   &\forall \sigma \in
...
\begin{theorem}[Type Soundness]\label{theorem:sound} Given a well-formed handler specification \(\Delta\), with ghost variables \(\overline{x{:}b}\) and a violation property \(A\), a controller \(e\) that satisfies \(\overline{x{:\rawnuot{b}{\top}}};\Delta;\emptyset \vdash e : \rg{\epsTL}{A}{\epsTL}\) will realize at least one trace consistent with \(A\), i.e., 
