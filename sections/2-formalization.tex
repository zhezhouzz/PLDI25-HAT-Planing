...
\begin{example}[Capability Context] The handler context $\Delta$ for
  our running examples augments the four specifications from
  \autoref{fig:spec} as follows:
  {\small
...
\mprooftr{77mm}
{
$\Delta(\eff{op}) =\langle \eff{gen}~\overline{x_i{:}t_i}\sarr
\rg{H}{\lastA\msg{op}{\phi}}{A \seqA F}, \Theta' \rangle$ \quad
...
\mprooftr{77mm}
{
  $\Delta(\eff{op}) =\langle \eff{obs}~\overline{x_i{:}t_i}\sarr
\rg{H}{\lastA\msg{op}{\phi}}{A \seqA F}, \Theta'\rangle$ \quad
...
relation $\Gamma \wellfoundedvdash \tau$ ensures, e.g., that all
...
since both the history and prophecy automata constrain the contexts in
...
capabilities to $\Theta$. In order to make an observation using
$\eff{op}$, \textsc{TObs} additionally requires that the capability
context has a corresponding capability ($\{\eff{op}\} \cup B$). The
standard subsumption rule \textsc{TSub} allows us to change the shape
of a type a term is being typed against. Controller always end in a
...
\begin{example}[Controller Typing] We provide an informal typing
  ...
  is typed using \textsc{TGen}. After retrieving the specification of
  $\eff{writeReq}$ from $\Delta$ and uses \textsc{TSub} to adjust it
  into a shape consistent with $\negSCA'$:
  {\footnotesize
    \begin{align*}
      &\underline{(\eff{writeReq}, \Code{x}\hasnty{int}
        \sarr \rg{\allTL}{\lastA\msg{writeReq}{v =
        \Code{x}}}{\finalA\msg{writeRsp}{v = \Code{x}}}) \in \Delta} \\
      & \underline{\Gamma \vdash \eff{writeReq}: \Code{x}\hasnty{int} \sarr
        \rg{\allTL}{\lastA\msg{writeReq}{v =
        \Code{x}}}{\finalA\msg{writeRsp}{v = \Code{x}}}
        \tag{\textsc{TEOp}}} \\
      &\Gamma \vdash \eff{writeReq}:
        \Code{x}\hasnty{int} \sarr \effLR{\epsTL}\
        \effLR{\lastA\msg{writeReq}{v = \Code{x}}}\ \effLR{A_2 \seqA
        \lastA\msg{writeRsp}{v = \Code{x}} \seqA A_4}
        \tag{\textsc{TSub}}
    \end{align*}
  }\noindent %
  \BD{The current definition of \textsc{TGen} does not allow
    \textsc{TSub} to be applied (although neither did the old one); we
    will want to patch up this example after the metatheory is
    finished. } Since the type of $\eff{writeReq}$ aligns with the
