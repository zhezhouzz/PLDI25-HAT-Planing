...
($\allA \msgA{put}{k\;v} \allA$) was executed before.  The
underapproximate interpretation of \tyName{} encodes a form of
...
$\eff{put}$ operation on $k$.\footnote{Note that an overapproximate
  ...
  require that a $\eff{get}$ operation on $k$ be able simultaneously
  observe every value $v$ previously $\eff{put}$ into $k$. This would
  mean that after performing $\eff{put}~1~2$ and $\eff{put}~1~3$, the
  result of $\eff{get}~1$ would be both \emph{both} $2$ and $3$, which
...
they should satisfy the preconditions of the SUT. The need to manually
writing effective generators is a well-known pain point~\cite{HC+24}
to wider adoption of PBT, and developing generators for properties
that impose deep structural or semantic constraints on inputs remains
a publication-worthy exercise~\cite{pbt-ifc, PCRH+11, FQL24}.

The challenges with writing PBT generators are further exacerbated
when the SUT is effectful and includes components may be opaque, e.g.,
...
and control dependencies relevant to the property being checked.  Some
examples of scenarios where tests are naturally expressed this way
include:
\begin{enumerate}
...
\item \textbf{Serialized inputs:} The SUT is a procedure that
  ...
  input, and operates over the resulting structure. The test generator
...
operations, and to ensure that programs in a domain-specific language
for writing effectful generators perform sequences of effects
conforming to those dependencies.  Using the specification given
above, a well-typed program written in our DSL would only generate
test sequences in which every $\eff{get}$ operation is preceded by at
least one $\eff{put}$ on the same key, while placing no other
constraints on the operations that might be occur between the two.
While this use of \tyName{}s enables us to check whether a
manually-written generator explores traces relevant to a target
property, the burden of writing such a generator still remains.  To
alleviate this overhead, we connect these two elements by defining a
type-guided program synthesis technique that automatically construct
DSL generator programs using \tyName{} specifications. The resulting
generators are guaranteed to produce traces that violate a target
property when executed angelically~\cite{BC+10}, and can thus evince a
concrete bug assuming the SUT provides similar responses when
performing effects.
...
synthesis of a test generator in \name{}'s DSL, which it then
transpiles into different target languages; thus far, we have
implemented Quickcheck~\cite{CH00} (OCaml) and P~\cite{DGJ+13}
targets.  Synthesized test generator programs are guaranteed to
produce (effectful) test sequences that respect the underapproximate
interpretation of the \tyName{} API specifications.  The rest of the
paper is organized as follows. In Section~\ref{sec:overview}, we
provide additional motivation for the problem along with a series of
examples to illustrate our approach.  Section~\ref{sec:formal}
presents the operational semantics and type system for our DSL that
exposes the underapproximate structure of \tyName{}s.
Section~\ref{sec:synthesis} describes the synthesis algorithm that
translates these types into \name{} programs.  To demonstrate the
utility of our approach, we provide a detailed evaluation over a broad
range of diverse benchmarks drawn from the domains given above; these
include generating tests for semantically complex data structure
properties such as well-typed simply-typed lambda calculus terms using
de Brujin indices, as well as real-world distibuted protocols deployed
in industry.  Sections~\ref{sec:related} and~\ref{sec:conc} present
