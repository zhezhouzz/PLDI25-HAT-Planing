...
\tyName{}s, a type abstraction for \emph{underapproximating effectful
  computations}.  A \tyName{} type judgement has the form $\vdash e : [H]\,t\,[F]$ where $H$ and $F$ qualify type $t$ with pre- ($H$) and post-
...
\citet{ZYDJ24}. Unlike their overapproximate predecessors,
...
\begin{wrapfigure}{l}{.54\textwidth}
  \vspace*{-.1in}
  \centering
...
\item We describe a trace-guided synthesis procedure that uses
  ...
  tested, to generate well-typed test generators.  \cbnewadding{%
    We evaluate this procedure by comparing the generators synthesized
    by \name{} to state-of-the-art baselines on benchmarks whose bugs
    require deep (effectful) semantic and structural properties of the
    target SUT to surface.}

%% \item \cbremoved{We provide a detailed evaluation study, comparing the
...
such as well-typed simply-typed lambda calculus terms using de Bruijn
indices, as well as real-world distributed protocols deployed in
industry.  \autoref{sec:related} presents related work and conclusions.
