...
types~\cite{RW24,WGJ24,ZMD+23}. \citet{RW24}, for example, introduces
...
evaluate. \citet{ZMD+23}, on the other hand, leverages notions of
...
respect to a context (i.e., trace) of previously performed $\eff{put}$s:
\begin{align*}
...
$\eff{put}$ operation on $k$.\footnote{Note that an overapproximate
  ...
  require that a $\eff{get}$ operation on $k$ be able to simultaneously
  observe every value $v$ previously $\eff{put}$ into $k$. This would
  mean that after performing $\msgA{put}{k_1\;2}$ and $\msgA{put}{k_1\;3}$, the
  result of $\msgA{get}{k_1}$ would be both \emph{both} $2$ and $3$, which
...
\item We develop a DSL for writing test generators that are typed
  using \tyName{}s.  The DSL, an extension of a typed
  $\lambda$-calculus with support for asynchronous effects,
  nondeterministic choice, and recursion, is sufficiently expressive
  to be applicable in a diverse range of applications,
...
\item We provide a detailed evaluation study, comparing the
  ...
  baselines.  Our results support our claim that underapproximate type
  specifications offer a promising approach to guide the synthesis of test
  generators that exercise deep semantic and structural properties of a
  target SUT.
...
\tyName{}s.  Section~\ref{sec:synthesis} describes the synthesis
algorithm that translates these types into \name{} programs.  To
demonstrate the utility of our approach, we provide a detailed
evaluation over a broad range of diverse benchmarks drawn from the
domains given above; these include generating tests for semantically
complex data structure properties such as well-typed simply-typed
lambda calculus terms using de Brujin indices, as well as real-world
distibuted protocols deployed in industry.  Sections~\ref{sec:related}
