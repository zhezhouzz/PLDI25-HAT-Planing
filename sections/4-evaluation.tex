...
\item[\textbf{Q3}:] Is \name{} \emph{efficient}? Is it able to
  ...
  of time?
  \cbnewadding{\item[\textbf{Q4}:] Is \name{} \emph{sensitive}? Is it able to
  synthesize meaningful generators for benchmarks with weaker specifications?}
\end{itemize} %
...
\midrule
\textsf{IFCLoad1}\cite{pbt-ifc} & {\scriptsize A well-behaved IFC program containing a $\Code{Load}$ command never leaks a secret.} & $8$ & $37$ & $2$ & $15.4$ & $11293.7^{\dagger}$ & $5.76$ & $16$ & $18$ & $930$\\
\midrule
\cbnewadding{\textsf{IFCLoad2}\cite{pbt-ifc}} & \cbnewadding{{\scriptsize \textsf{IFCLoad1} without searching bias (cf. \autoref{sec:overview}).}} & \cbnewadding{$8$} & \cbnewadding{$33$} & \cbnewadding{$2$} & \cbnewadding{$115.4$} & \cbnewadding{$11293.7^{\dagger}$} & \cbnewadding{$4.83$} & \cbnewadding{$16$} & \cbnewadding{$18$} & \cbnewadding{$754$}\\
\midrule
...
\emph{well-typed higher-order} STLC program that uses a de Brujin representation.} & $10$ & $118$ & $4$ & $1.4$ & \timeout{}$^{\dagger}$ & $91.73$ & $18$ & $31$ & $9034$\\
\midrule
\cbnewadding{\textsf{DeBruijn3}\cite{CoverageType}} & \cbnewadding{{\scriptsize \textsf{DeBruijn2} with \emph{untyped higher-order} programs (cf. \autoref{sec:overview}).}} & \cbnewadding{$10$} & \cbnewadding{$107$} & \cbnewadding{$4$} & \cbnewadding{\timeout{}} & \cbnewadding{\timeout{}$^{\dagger}$} & \cbnewadding{$3.23$} & \cbnewadding{$7$} & \cbnewadding{$22$} & \cbnewadding{$957$}\\
\midrule
\midrule
...
\midrule
\textsf{Transaction1} & {\scriptsize Asynchronous read operations are logically atomic.} & $8$ & $45$ & $3$ & $1.2$ & \timeout{} & $2.89$ & $15$ & $16$ & $423$\\
\midrule
\cbnewadding{\textsf{Transaction2}} & \cbnewadding{{\scriptsize \textsf{Transaction1} with weaker specs (cf. \autoref{sec:overview}).}} & \cbnewadding{$8$} & \cbnewadding{$40$} & \cbnewadding{$3$} & \cbnewadding{\timeout{}} & \cbnewadding{\timeout{}} & \cbnewadding{$2.24$} & \cbnewadding{$15$} & \cbnewadding{$15$} & \cbnewadding{$372$}\\
\midrule
...
from. \cbnewadding{The \textsf{IFCLoad2}, \textsf{DeBruijn3}, and \textsf{Transaction2} benchmarks are variants of the original ones with weaker specifications; we omit them untils the detailed discussion at the end of this section (\textbf{Q4}).}
...
\cbnewadding{ \paragraph{Input Quality Sensitiveness} The effectiveness and the efficiency of
\name{} depend on the quality of the user-supplied \tyName{}
specifications. As discussed in \autoref{sec:overview},
Overly loose \tyName{}s offer synthesis little guidance; in the limit, exploration approaches random testing.
To quantify \name{}'s sensitivity to specification quality, we ran an
additional study on our OCaml and P benchmarks, keeping the SUT and the
global property fixed while weakening only the \tyName{}s of
individual operators.
\autoref{tab:evaluation} reports the outcome for the three motivating
examples from \autoref{sec:overview}\footnote{The full results are given in the supplemental material.}.
The weakening following the description in \autoref{sec:overview}, for example, \textsf{IFCLoad2} weakens the ``valid address'' constraint (i.e., $\I{isAddr}$) from the original specification.
We consider such specifications \emph{lower-quality} because their lack of guidance increases the risk that \name{} will synthesize generators misaligned with SUT's intended behavior.
Under these weaker specifications, average synthesis time (t$_\text{total}$) decreases by $18.18\%$ on average, and by $78.07\%$ in the extreme case (\textsc{ChainReplication}). However, $26.9\%$ (7 out of 26) of the benchmarks time out; the remaining benchmarks show an increasing retry rate (\#Num. Executions), by $1.9\times$ on average and $7.5\times$ in the extreme case (\textsc{IFCLoad}).}

\cbnewadding{We learn from the experimental results that this sensitivity reflects a rough split between basic correctness and search bias. Weakening basic correctness therefore affects \name{} more severely (e.g.,\textsf{DeBruijn3} generates untyped STLC programs to test an interpreter with type-checking): synthesized generators tend to yield large volumes of executions that are poorly matched to how the SUT is meant to be exercised, so obtaining an informative witness
may require far more effort or time out entirely.
Conversely, if basic correctness constraints are too strong overall, the synthesizer may not find effective test generators; in the extreme, conflicting \tyName{}s across operators leave synthesis without a solution.
Weakening only search biases is comparatively benign (e.g.,
\textsf{IFCLoad2}), mainly increasing retries before a bug is found, and
because weaker specifications often shorten synthesis, that loss of
testing efficiency is sometimes an acceptable trade-off.}
