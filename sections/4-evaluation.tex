...
\cbnewadding{%
  ...
  the SUT or runtime rejects a candidate trace (e.g., the interpreter
  aborts on an ill-typed fragment), we treat that run as uninformative
  and \emph{retry} with a fresh sample rather than counting it toward a
  found violation.  This matches how generators are typically used in
  practice and avoids conflating specification imprecision with genuine
  safety failures.}
...
\midrule
\cbnewadding{\textsf{DeBruijn2}$_{\mathsf{W}}$\cite{CoverageType}} & \cbnewadding{{\scriptsize \textsf{DeBruijn2} with \emph{untyped higher-order} programs (cf. \autoref{sec:overview}).}} & \cbnewadding{$10$} & \cbnewadding{$107$} & \cbnewadding{$4$} & \cbnewadding{\timeout{}} & \cbnewadding{\timeout{}$^{\dagger}$} & \cbnewadding{$3.23$} & \cbnewadding{$7$} & \cbnewadding{$22$} & \cbnewadding{$957$}\\
\midrule
\midrule
...
by others (\textbf{Q1}).\footnote{%
  \cbnewadding{%
  As anecdotal evidence in the affirmative, the \tyName{}s for the $\textsc{HashTable}$ and $\textsc{Courseware}$ benchmarks in \autoref{tab:evaluation} were written by an undergraduate student, and those for $\textsc{Smallbank}$ and $\textsc{Twitter}$ by two first-year PhD students. All three students were new to our \tyName{} specification language and had no knowledge of our synthesis algorithm. After a couple of hands-on training sessions, each was able to independently encode their knowledge about the SUT as \tyName{}s, requiring only a few person-hours to complete all the benchmarks. }}  Our benchmarks are grouped into four
...
from. \cbnewadding{The \textsf{IFCLoad}$_{\mathsf{W}}$, \textsf{DeBruijn2}$_{\mathsf{W}}$, and \textsf{Transaction}$_{\mathsf{W}}$ benchmarks are variants of the original ones with weaker specifications; we provide
additional discussion of these at the end of this section (\textbf{Q4}).}
...
generator available\footnote{We used the artifacts available in their
  ...
  Bruijn}), \tyName{}-guided synthesis leads to significantly better
...
\cbnewadding{ \paragraph{Specification Quality Robustness} \name{}'s
  effectiveness depends on the quality of user-supplied \tyName{}
  specifications.  To quantify \name{}'s robustness to specification
  quality, we ran an additional study on our OCaml and P benchmarks,
  keeping the SUT and the global property fixed while weakening the
  \tyName{} specifications of individual operators in the SUT.
  \autoref{tab:evaluation} summarizes the outcomes for the three
  motivating examples presented in~\autoref{sec:overview}\footnote{
  Details for the full set of benchmarks are provided in the supplemental material.}.  The
  specifications are weakened following the description in
  \autoref{sec:overview}, for example, \textsf{IFCLoad}$_{\mathsf{W}}$ eliminates
  the ``valid address'' constraint (i.e., $\I{isAddr}$) from the
  original specification.  We consider such specifications
  \emph{lower-quality} because their lack of guidance increases the
  risk that \name{} will synthesize generators misaligned with SUT's
  intended behavior.  Under these weaker specifications, average
  synthesis time (t$_\text{total}$) decreases by $18.18\%$ on average,
  and by $78.07\%$ in the extreme case
  (\textsc{ChainReplication}). However, $26.9\%$ (7 out of 26) of the
  benchmarks time out; the remaining benchmarks show an increasing
  retry rate (\#Num. Executions), by $1.9\times$ on average and
  $7.5\times$ in the extreme case (\textsc{IFCLoad}$_{\mathsf{W}}$).}

\cbnewadding{Our ablation study reveals that weakening basic
  correctness properties severely impacts \name{}'s effectiveness
  (e.g., \textsf{DeBruijn2}$_{\mathsf{W}}$ generates untyped STLC programs to test an
  interpreter with type-checking).  This is because generators
  synthesized from such specifications yield a large number of
  executions that are poorly matched to how the SUT is meant to be
  exercised; obtaining an informative witness may thus require far
  more effort or time out entirely.  Conversely, weakening search bias
  is comparatively benign (e.g., the synthesized generator for
  \textsf{IFCLoad}$_{\mathsf{W}}$ still finds EENI violations), primarily
  manifesting as increased retries before a bug is found.  On the
  other hand, because weaker specifications can lead to shorter
  synthesis times, this loss of testing efficiency may sometimes be an
