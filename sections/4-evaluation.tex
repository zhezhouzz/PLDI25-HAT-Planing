...
our DSL can be transpiled to OCaml (QCheck) or P.  Our experiments
...
\midrule
\textsf{ReaderWriter} & {\scriptsize Asynchronous read operations are logically atomic.} & $8$ & $45$ & $3$ & $1.2$ & {\tiny Timeout} & $2.89$ & $15$ & $16$ & $423$\\
\midrule
...
Name & Property &   &uHAT &goal & {\scriptsize Clouseau} & {\scriptsize Baseline} &  &  \\
  \midrule
  {\scriptsize\textsf{Database}} & {\scriptsize The database maintains a Read-Your-Writes policy.} & $4$ & $15$ & $3$ & $1$ & $6$ & $0.32$ & $6$ & $6$ & $49$\\
  \midrule
  {\scriptsize\textsf{Firewall}}\cite{MessageChain} & {\scriptsize  Requests generated inside the firewall are eventually propagated to the outside.} & $5$ & $26$ & $4$ & $1$ & $10$ & $0.59$ & $10$ & $10$ & $222$\\
  \midrule
  {\scriptsize\textsf{RingLeaderElection}}\cite{MessageChain} & {\scriptsize There is always a single unique  leader.} & $3$ & $14$ & $2$ & $1$ & $18$ & $0.83$ & $8$ & $8$ & $100$\\
  \midrule
  {\scriptsize\textsf{BankServer}}\cite{DGJ+13} & {\scriptsize Withdrawals in excess of the available balance are never allowed.} & $6$ & $42$ & $1$ & $1$ & $2^{\dagger}$ & $0.17$ & $5$ & $5$ & $27$\\
  \midrule
  {\scriptsize\textsf{Simplified2PC}}\cite{DGJ+13} & {\scriptsize Transactions are atomic.} & $9$ & $32$ & $5$ & $2$ & $6^{\dagger}$ & $1.03$ & $8$ & $8$ & $88$\\
  \midrule
  {\scriptsize\textsf{HeartBeat}}\cite{DGJ+13} & {\scriptsize All available nodes are identified by a  detector.} & $7$ & $31$ & $3$ & $1$ & $6^{\dagger}$ & $1.10$ & $14$ & $14$ & $145$\\
  \midrule
  {\scriptsize\textsf{ChainReplication}}\cite{ModP} & {\scriptsize Concurrent updates are never lost.} & $7$ & $72$ & $4$ & $1$ & $500^{\dagger}$ & $19.24$ & $12$ & $169$ & $2054$\\
  \midrule
  {\scriptsize\textsf{Paxos}}\cite{ModP} & {\scriptsize Logs are correctly replicated.} & $10$ & $110$ & $2$ & $1$ & $667^{\dagger}$ & $23.70$ & $14$ & $77$ & $1763$\\
  \midrule
  {\scriptsize\textsf{Raft}} & {\scriptsize Leader election is robust to faults.} & $9$ & $39$ & $6$ & $1$ & {\tiny Timeout} & $26.84$ & $12$ & $78$ & $1262$\\
  \midrule
  {\scriptsize\textsf{AnonReadAtomicity}} & {\scriptsize Read Atomicity is preserved.} & $17$ & $113$ & $3$ & $1$ & $53^{\dagger}$ & $18.35$ & $16$ & $16$ & $1909$\\
\bottomrule
  \end{tabular}
...
\textsf{Courseware}, \textsf{Twitter}, \textsf{Smallbank}).
Violations of these properties manifest in specific configurations
(e.g., a violating test case for \textsf{NFA} involves constructing an
automata containing nodes with more than two outgoing edges).  We
...
anywhere from $6$ - $22$ different operators which occur in over 100
symbolic events for the most complex examples.  The second group of
...
outcomes.  The baseline for A number of the concurrency benchmarks is
MonkeyDB~\cite{MonkeyDB}, a mock storage system that can be used to
find violations of database isolation properties.  Although it is
specialized for this particular application class, and \name{} is not,
evaluation results are comparable, with \name{} yielding generators
that identify weak isolation violations modestly more effectively than
the targeted baseline.  Similar conclusions apply to the P benchmarks
shown in~\autoref{tab:evaluation2}.  For the simple benchmarks in this
...
in~\ref{sec:overview}. The property to be checked requires that if there
exists a key $\Code{k}$ updated within an active transaction
$\Code{i}$, any successful read response asking its value should
return the value last written to $\Code{k}$ made by that transaction.
%% We can express a
...
executions, compared to \name{}'s version which is able to uncover the
violation on all of its executions.
