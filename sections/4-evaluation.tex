...
\midrule
\cbnewadding{\textsf{Transaction2}} & \cbnewadding{{\scriptsize \textsf{Transaction1} without read atomicity enforcement (cf. \autoref{sec:overview}).}} & \cbnewadding{$8$} & \cbnewadding{$40$} & \cbnewadding{$3$} & \cbnewadding{\timeout{}} & \cbnewadding{\timeout{}} & \cbnewadding{$2.24$} & \cbnewadding{$15$} & \cbnewadding{$15$} & \cbnewadding{$372$}\\
\midrule
...
from. \cbnewadding{The \textsf{IFCLoad2}, \textsf{DeBruijn3}, and \textsf{Transaction2} benchmarks are variants of the original ones with weaker specifications; we provide
...
\cbnewadding{ \paragraph{Specification Quality Robustness} \name{}'s
  ...
  \autoref{sec:overview}, for example, \textsf{IFCLoad2} eliminates
  ...
  $7.5\times$ in the extreme case (\textsc{IFCLoad2}).}

\cbnewadding{Our ablation study reveals that weakening basic
  correctness properties severely impacts \name's effectiveness
  (e.g.,\textsf{DeBruijn3} generates untyped STLC programs to test an
  interpreter with type-checking).  This is because generators
  synthesized from such specification yield a large number of
  ...
  more effort or time out entirely.  Converesely, weakening search
  biases is comparatively benign (e.g., the synthesized generator for
  \textsf{IFCLoad2} still finds read atomicity violations), primarily
