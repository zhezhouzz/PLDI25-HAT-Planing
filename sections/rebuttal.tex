...
systems (Reviewers E); we elaborate on this point in the individual responses, and
...
% Need to Read: Finding Race Conditions in Erlang with QuickCheck and
% PULSE, ICFP 2009
...
\paragraph{Network Behavior} In-order and out-of-order delivery can be specified using the prophecy automata in the \PAT{} of each handler. Suppose a handler sends event \(\eff{E_1}\) followed by \(\eff{E_2}\). The following \PAT{} enforces in-order delivery by ensuring that \(\eff{E_1}\) is received before \(\eff{E_2}\):
    \begin{align*}
...
\paragraph{\textsc{RingLeaderElection}} The benchmark models a leader election algorithm. Each nomination message $\eff{nominate}(\I{s},\I{d},\I{l})$ transmitted by the model's actors contain a source ($\I{s}$), destination ($\I{d}$), and the nominated leader ($\I{l}$). The actor handles incoming nomination message following three \PAT{}s: 
\begin{enumerate}
    \item When a node $\Code{m}$ receive a message that nominates itself as leader ($\Code{ld = m}$) from another node $\Code{n}$, the node $\Code{m}$ can announce itself to be the leader via $\eff{won}$ message.
    {\small\begin{align*}
  &\Code{n}\hasoty{tNode}{\top} \sarr \Code{m}\hasoty{tNode}{\top}
  \sarr \Code{ld}\hasoty{tNode}{\vnu = \Code{m}} \sarr
  \\&\rg{\globalA\evparenth{\top}}{\lastA\msg{nominate}{
      \I{s} = \Code{n} \land \I{d} = \Code{m} \land \I{l} =
      \Code{ld} }}{\finalA\msg{won}{\I{l} =
      \Code{m}}}
\end{align*}}\noindent
    \item When a node $\Code{m}$ receive a message that nominates a leader has larger index than it ($\Code{ld > m}$), the node $\Code{m}$ will also nominate the leader in the incoming message.
    {\small\begin{align*}
  &\Code{n}\hasoty{tNode}{\top} \sarr \Code{m}\hasoty{tNode}{\top}
  \sarr \Code{ld}\hasoty{tNode}{\vnu > \Code{m}} \sarr
  \\&\rg{\globalA\evparenth{\top}}{\lastA\msg{nominate}{\I{s} = \Code{n} \land \I{d} = \Code{m} \land \I{l} = \Code{ld} }}{\finalA\msg{nominate}{\I{s} = \Code{m} \land \I{d} = \I{next}(\Code{m}) \land \I{l} = \Code{ld}}}
\end{align*}}\noindent
The predicate $\I{next}(n_1, n_2)$ indicates that node $n_2$ is the next node of $n_1$ in the ring.
\item When a node $\Code{m}$ receive a message that nominates a leader has less index than it ($\Code{ld < m}$), the node $\Code{m}$ will nominate itself as leader.
    {\small\begin{align*}
  &\Code{n}\hasoty{tNode}{\top} \sarr \Code{m}\hasoty{tNode}{\top}
  \sarr \Code{ld}\hasoty{tNode}{\vnu < \Code{m}} \sarr
  \\&\rg{\globalA\evparenth{\top}}{\lastA\msg{nominate}{\I{s} = \Code{n} \land \I{d} = \Code{m} \land \I{l} = \Code{ld} }}{\finalA\msg{nominate}{\I{s} = \Code{m} \land \I{d} = \I{next}(\Code{m}) \land \I{l} = \Code{m}}}
\end{align*}}\noindent
\end{enumerate}
Then, a node will handle the incoming nomination message following three intersected \PAT{}s: 
{\small\begin{align*}
  &\Code{n}\hasoty{tNode}{\top} \sarr \Code{m}\hasoty{tNode}{\top}
  \sarr \Code{ld}\hasoty{tNode}{\vnu = \Code{m}} \sarr
  \\&\rg{\globalA\evparenth{\top}}{\lastA\msg{nominate}{\I{s}\;\I{d}\;\I{l}}{
      \I{s} = \Code{n} \land \I{d} = \Code{m} \land \I{l} =
      \Code{m} }}{\finalA\msgB{won}{\I{l}}{\I{l} =
      \Code{m}}}
\end{align*}}\noindent
This specification indicates that when a node $\Code{m}$ receive a message that nominate
($\eff{nominate}$) itself as leader ($\I{l} = \Code{m}$) from
another node $\Code{n}$, then node $\Code{m}$ can announce itself to
be the leader via $\eff{won}$ message.
{\small\begin{align*}
  &\Code{n}\hasoty{tNode}{\top} \sarr \Code{m}\hasoty{tNode}{\top}
  \sarr \Code{ld}\hasoty{tNode}{\vnu > \Code{m}} \sarr
  \\&\rg{\globalA\evparenth{\top}}{\lastA\msgB{nominate}{\I{s}\;\I{d}\;\I{l}}{
      \I{s} = \Code{n} \land \I{d} = \Code{m} \land \I{l} =
      \Code{m} }}{\finalA\msgB{nominate}{\I{s} = \Code{m} \land \I{d} = \I{next}(\Code{m}) \land \I{l} =
      \Code{ld}}}
\end{align*}}\noindent

The protocol tries to nominate an unique leader among a set of nodes (with type $\Code{tNode}$), and there is a predicate $\I{next}(n_1, n_2)$ indicates that $n_2$ is the next node of $n_1$, which guarantee these nodes are connected as a ring. We omit the dest In order to start the leader nominateion procedure, the controller needs to wake up node $n$ via message $\eff{eWakeup}(n)$:
{\small\begin{align*}
    \eff{eWakeup}: n{:}\Code{tNode}\sarr \rg{\allTL}{\msg{eWakeup}{\I{nd} = n}}{\finalA \msg{nominate}{\I{nd} = n \land \I{l} = n}}
\end{align*}}\noindent
As shown in the prophecy automata, the node will send nominate message 