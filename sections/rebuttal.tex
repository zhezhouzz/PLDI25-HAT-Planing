...
Lamport's sequential consistency property with a simple example where
multiple nodes (processes) perform read and write operations on a
database. Each node communicates with the database using
\(\eff{readReq/Resp}\) and \(\eff{writeReq/Resp}\) events, which
...
\(\eff{gReadReq/Resp}\) and \(\eff{gWriteReq/Resp}\). Then sequential
  ...
  {\small\begin{align*}
     ...
     \item The execution remains the same as if all operations were
       ...
       {\small\begin{align*}
        ...
        \\ & i{:}\Code{rId} \sarr \Code{x}\hasnty{int} \sarr \rg{(\finalA \msg{writeReq}{v = \Code{x}} \land \nextA \neg \eff{writeReq} \untilA \msg{readResp}{v = \Code{x} \land \I{id} = i }) }{\lastA \msg{readResp}{\I{id} = i}}{\allTL}
    \end{align*}}\noindent
  where the \PAT{} of $\eff{readReq}$ requires the value read to be
  last written value according to the controller's view
  ($\eff{gWriteReq}$), and the \PAT{} of $\eff{readResp}$ requires it
  to be the last value of  under actor's view
  ($\eff{writeReq}$).
...
progress. A node \( n \) will use the \(\eff{aquire}(n, r)\) and
