...
We provide three additional examples to address the expressivity concerns from Reviewer E: 1) "exactly one event"; 2) "multiple traces per thread" properties (i.e., sequential consistency); 3) network behaviors. We also elaborate all \PAT in our benchmarks (e.g., 2PC and RingLeaderElection required by Reviewer E).
...
\paragraph{Network Behavior} Suppose an actor sends event \(\eff{E_1}\) followed by \(\eff{E_2}\). 
\begin{enumerate}
    ...
    $}
\end{enumerate}
Node failure can be represented as a special event, \(\eff{crash}\), sent by the controller. An handler can handle messages normally only if it has not received the \(\eff{crash}\) event:
{\small\begin{align*}
...
\paragraph{\textsc{RingLeaderElection}} The benchmark consistent with three events: 1) message $\eff{nominate}(\I{s},\I{d},\I{l})$ indicate node $\I{s}$ send message to node $\I{d}$ to nominate node $l$ as leader; 2) message $\eff{won}(\I{l})$ announce nodes $\I{l}$ is a leader; 3) message $\eff{wakeup}(\I{n})$ enables node $n$ to elect a leader. The unique leader property requires there are no two different nodes are announced to be leader. The \PAT{}s are shown as below:
{\small\begin{align*}
...
\end{align*}}\noindent
The predicate $\I{next}(n_1, n_2)$ indicates that node $n_2$ is the next node of $n_1$ in the ring. The three intersected \PAT of $\eff{nominate}$ indicates two situations when a node receives a nomination message:
\begin{enumerate}
    \item if nominated leader has an equal id, announce itself as leader
    \item if nominated leader has an different id, nominate this leader
\end{enumerate}
The protocol is wrong with respect to unique leader property, since it should only agree with the input nomination message when nominated leader has greater id instead of different id. 

\paragraph{\textsc{Simplified2PC}} In this simplified version of two-phrase-commit proposal, we assume transactions have a single update operation. The controller communicate with coordinator via $\eff{read/write}$ messages, and coordinator communicates with underline database via $\eff{put/get}$ messages. Depends on the response of database, the coordinator can $\eff{commit/abort}$ one transaction. The \PAT{}s of $\eff{readRsp/writeRsp/commit/abort}$ are not interesting since handling these messages doesn't lead new sending message, thus we omit them. The \PAT{}s of rest operators are defined as the following:
{\small\begin{align*}
...
\end{align*}}\noindent
Notice that the database handles $\eff{getReq}$ message by returns the default value ($\minus 1$) when there is no commit yet, otherwise return the last commit one. The coordinator handles $\eff{getReq}$ message but sending write response to controller and sending commit/abort to database at \emph{the same time}, which leads violation of RYW property. 

\paragraph{\textsc{Database}} We already introduced it as our motivating example.

\paragraph{\textsc{Firewall}} A set of internal and external nodes communicating through a firewall. Firewall should block message from an external node, unless this node has received message from internal nodes previously. The firewall actually keep a whitelist of external nodes that can communicate with internal nodes.
{\small\begin{align*}
...
\end{align*}}\noindent
Notice that this whitelist only store the most recent external node (first \PAT of $\eff{externalReq}$), which violates (informal) liveness property that all message should eventually have response.
...
\end{align*}}\noindent
When the coffee machine handles $\eff{eGrindBeansReq}$ and $\eff{eStartEspressoReq}$ message, it will non-deterministically send $\eff{eNoWaterError}$ and $\eff{eNoBeansError}$ to the panel; the panel then will directly report them to controller via $\eff{eCoffeeMakerError}$. 