...
\vspace{-.4cm} \par\nobreak {\small
  \begin{flalign*}
    ...
    \text{\textbf{Event Set}} \quad \Pi ::= \emptyset ~|~
    \pi ~|~ \Pi \cup \Pi
  \end{flalign*}}\noindent Each symbolic events $\msg{op}{\phi}^b$ in
an abstract trace is tagged with a boolean $b$ indicating whether the
surrounding context satisfies its constraints. Every SRE can be
transformed into a set of abstract traces;\footnote{The definition of
  our SRE normalization procedure is provided in the \techreport{}.}
operating over abstract traces helps our synthesizer easily identify
events with unresolved dependencies. \BD{I'm still iffy on why these
  need to be defined mutually recursively.}

\begin{example}[SRE Normalization]\label{ex:sre-norm}
  The normalized version of the history regex of $\eff{pop}$ from
  Example~\ref{ex:stack-example} includes the following abstract
  trace: %
  \vspace{-.4cm} \par\nobreak {\small %
    \begin{align*}
    ...
    \end{align*}}\noindent
  The normalized trace assumes that the three combine
  symbolic events in the original SRE occur in the order
  $\msgAGray{popI}{m\;\_}$, $\msgAGray{pushI}{(n-m)\;x}$, and
  $\msgAGray{pushI}{n\;\_}$. The Kleene stars after
  $\msgAGray{popI}{m\;\_}$ and $\msgAGray{pushI}{(n-m)\;x}$ in the
  trace are needed to disallow the $\msgCGray{popI}$ event and
  $\msgCGray{pushI}$ events, respectively.  The trace shown above is
  one possible normalized result.  By assuming $m = 0$, we can make
  two $\effGray{pushI}$ events to be the same event, resulting in the
  another normalized trace: \BD{Assuming that $m = 0$ seems pretty
    magical-- does our normalization procedure actually do this?} %
  \vspace{-.4cm} \par\nobreak {\small %
\begin{align*}
...
satisfied; it then uses the $\refine$ subroutine to insert the
...
specification \BD{The careful reader will notice we're using a very
  ...
  that's okay?}.  Before returning the refined trace, \refine{} checks
that it corresponds to a meaningful test generator by filtering out
...
\begin{example}[Refinement Process]\label{ex:refinement-process}
  ...
  \end{align*}} %
\noindent The new variables in the type context require $m = 0$, as
