...
test, Corollary~\ref{theorem:sound} ensures that a well-typed test generator
...
$\Delta$, and b) has a \tyName{} whose future automaton shares a
...
after that event so that its dependencies are satisfied, ensuring that
...
element of this set represents a generator that produces at least one
feasible trace. After the refinement loop has finished, a well-typed
generator program can be mechanically extracted using the refined
...
specification.
Before returning the refined trace, \refine{} checks it
corresponds to a meaningful test generator by filtering out empty
traces (line $7$).
...
\vspace{-.4cm} \par\nobreak{\small %
  \begin{align*}
    ...
    \\&\;\Pi_\Code{noI}^* \seqA \msgAGray{pushI}{m\;y}^\bot
...
generator, it iteratively uses \deriveTrace{} to fill in the holes of a template recursive function definition. It does so by associating each of the
