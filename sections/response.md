...
+ A: Our framework uses an SMT solver in two places: first, a solver is
...
and well-studied TLA+ toolchain) and automated testing frameworks
(e.g., Quickstrom).
...
variables. Second, our DSL's nondeterministic choice operator `⊕`,
...
checkers, frame rules, and other mechanisms to minimize redundant and
uninteresting exploration.  For these reasons, we choose to trade off
