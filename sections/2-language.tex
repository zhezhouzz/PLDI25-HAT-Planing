...
in the style of solver-aided languages~\cite{BC+10}. As we saw in
\autoref{sec:overview}, asynchronous effects can be modeled as a pair
of $\eff{opReq}$ / $\eff{opResp}$ operation, where the former
generates an id (given as $\iota$ in the syntax) that can then be
subsequently used when performing a corresponding response operation.
...
fields of an effectful operation are clear from context, we omit its
arguments $\overline{x}$, e.g.,
$\langle\textbf{write}\;|\;v > 0\rangle$ means
$\langle\textbf{write}\;\iota\;v\;|\;v > 0\rangle$. We
omit quantifiers with equality constraints, e.g.,
$\langle\textbf{write}\;\iota\;v\;|\;\iota = 1 \land v = 3\rangle$, as
$\msgA{write}{1\;3}$; we also omit the top ($\top$) qualifier in
...
\begin{example}[Operator Application Typing]\label{ex:operator-application-typing}
  ...
  number $x$ using the generator \Code{pos\_gen} whose type is
