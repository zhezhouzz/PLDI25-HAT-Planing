...
{\footnotesize
  \begin{alignat*}{2}
    ...
    % ~|~ \eff{obs}
    \\[-0.2em]\text{\textbf{Events}} & \quad & m ::= \quad & \eff{op}{\; \overline{v}} ~|~ \reqOp{op}{\ensuremath{\langle \iota,\overline{v}\rangle}} ~|~ \respOpAlt{op}{\ensuremath{\iota}}
...
in the style of solver-aided languages~\cite{Rosette}. Effectful
...
The reduction rules are shown in \autoref{fig:selected-semantics}: a pure operation application does not change the trace, while an effectful operation application generates a new event whose payload contains both the arguments and the return value.
...
contains a matching $\req$ with the same id. We also requires the history regex $H$ is not empty, which guarantees the it represents meaningful underapproximate preconditions.
...
Therefore, we expect that the type provided by the operator context is the most precise one, i.e., it doesn't allow any unreachable history traces.
...
The application rule \textsc{TEffOp} requires the arguments of an effectful operator to be well-typed against the parameter types, and the \textsc{TRspOp} rule requires the response of an effectful operator to be well-typed against the return type. The rule \textsc{TLet} requires the binding expression $e_1$ to generate the first part of its future regex $F_1$, then the rest of regex should be produced by the body expression $e_2$ under new history regex that appending with $F_1$.
...
\begin{example}[Operator Application Typing]
  ...
  The rest of program should be typed via the following judgement:
  \vspace{-.4cm} \par\nobreak {\small %
    \begin{align*}
    ...
    \end{align*}}%
  \noindent
  Similarly, we can instantiate the ghost varaible $n$ as $0$ and $m$ as $0$ via rule \textsc{SubG}, align the return type via rule \textsc{SubHF} and refine the history regex via the rule \textsc{TOpHis}.
  \vspace{-.4cm} \par\nobreak {\footnotesize %
    \begin{flalign*}
...
rule which merges \tyName{} of the two branches.  The typing
rule for function (\textsc{TFun}) is standard, and we can type the recursive function by unrolling similar to the incorrectness logic.
The \textsc{FixBase} rule types any recursive
...
Although ghost events is constrained by \tyName{}s, it doesn't show up in the actual traces produced by the program. We define a erasure function $\eraseGhost{\alpha}$ to erase the ghost events from the trace $\alpha$.
...
We expect the \tyName{}s provided by the operator typing context $\Delta$ to be consistent with corresponding auxiliary operator semantics, defined as the following.
\begin{definition}[Well-formed operator typing context]
  ...
  consistent with the type $\overline{x{:}b_{\I{x}}}\garr
    \overline{y{:}t_{\I{y}}}\sarr \uhat{H}{z{:}t}{F}$ provided by $\Delta$, and corresponding history regex doesn't contain any unreachable history traces.
  \begin{align*}
\overline{\forall
