...
\emph{must} evaluate to.  For example, the constant value $1$ can be
assigned the types $\ort{\Int}{\vnu = 1}$ and $\ort{\Int}{0 < \vnu}$
-- it satisfies constraint in both \emph{overapproximate} qualifiers
-- but, it cannot be assigned the type, $\urt{\Int}{\vnu = 1 \lor \vnu
  2}$, since the value 1 cannot evaluate to both 1 or 2 as required
...
subtyping rule for \tyName{}s, \textsc{SubHF}.  This rule uses on an
auxiliary inclusion relation on SREs $\Gamma \vdash A \subseteq A$ to
check relate the history and future regexes of two \tyName{}s under
the current type context $\Gamma$. Notably, unlike (overapproximating)
