...
{\footnotesize
  \begin{alignat*}{2}
    ...
    \\[-0.2em]\text{\textbf{Constants }}& \quad &c ::= \quad & () ~|~ \mathbb{B} ~|~ \mathbb{Z} ~|~ ...
    ...
    \\[-0.2em]\text{\textbf{Effectful Operations}}& \quad & \eff{op} ::= \quad & \eff{read} ~|~ \eff{write} ~|~ ...
    % \\\text{\textbf{Event Kinds}}& \quad & k ::= \quad & \eff{gen}
    % ~|~ \eff{obs}
    \\[-0.2em]\text{\textbf{Events}} & \quad & m ::= \quad & \langle\eff{op}{\; \overline{v}}\rangle ~|~ \reqOp{op}{\ensuremath{\iota\;\overline{v}}} ~|~ \respOpAlt{op}{\ensuremath{\iota}}
    \\[-0.2em]\text{\textbf{Values }}& \quad & v ::= \quad & c ~|~ x ~|~ \zlam{x}{s}{e} ~|~ \zfix{f}{s}{x}{s}{e}
    \\[-0.2em]\text{\textbf{Expressions}}& \quad & e ::=\quad & v ~|~ e
...
other calculii for trace-based type systems~\cite{ZYDJ24}; see
\autoref{fig:selected-semantics}.\footnote{The full set of reduction
  rules are included in the appendix.} The reduction relation
...
output trace of an effectful operation is an event whose payload
contains both the arguments and the return value of the operation.
...
In contrast to traditional type systems where a pure type qualifier
$\ort{b}{\phi}$ constrains every value a pure expresion \emph{may}
evaluate to; the type qualifier $\urt{b}{\phi}$ constrains a subset of
the values it \emph{must} evaluate to.  For example, the constant
value $1$ can be assigned the types $\ort{\Int}{\vnu = 1}$ and
$\ort{\Int}{0 < \vnu}$ -- the value 1 satisfies the constraint in both
\emph{overapproximate} qualifiers -- but, 1 cannot be assigned the
type, $\urt{\Int}{\vnu = 1 \lor \vnu = 2}$, since it cannot evaluate
to both 1 and 2 as required by this \emph{underapproximate}
qualifier. The term $1 \oplus 2 \oplus 3$, in contrast, does have this
...
omit its arguments $\overline{x}$, e.g., $\textsf{Req}\,(\langle\textbf{write}\;|\;v
  > 0\rangle)$ means $\textsf{Req}\,(\langle\textbf{write}\;\iota\;v\;|\;v > 0\rangle)$. We omit
quantifiers with equality constraints, e.g.,
$\langle\textbf{write}\;\iota\;v\;|\;\iota = 1 \land v = 3\rangle$, as
...
\subsubsection{Example}\label{ex:stack-example}
  ...
  {\small\begin{align*} &\eff{push} :~ n{:}\Int \garr y{:}\Int \garr
    x{:}\urt{\Int}{y < \vnu} \sarr
    \uhat{\Code{Last}(\msgAGray{pushI}{n\;y})}{\Unit}{\msgA{push}{x}\msgAGray{pushI}{n{+}1\;x}}
    \\&\eff{pop} :~ n{:}\Int \garr m{:}\Int \garr 
    \\[-0.4em]& \quad \uhat{\Code{Last}(\msgAGray{popI}{m\;\_}) \land
...
{TRspOp}
{$\Gamma\vdash \respOpAlt{op}{\ensuremath{\iota}} : \uhat{H}{t}{\respEvt{op}{\ensuremath{\iota\; \overline{v_i}}} \seqA F}$}
\\[.8em]
\mprooftr{35mm}
{
...
typing context $\Gamma$; that the verification conditions generated by
...
contains a matching $\req$ with the same id. We also require the
...
subtyping rule for \tyName{}s.  The rule \textsc{SubG} allows the
instantiation of a ghost variable in a \tyName{} into well-formed
types. The rule \textsc{SubHF} uses an auxiliary inclusion relation on
SREs $\Gamma \vdash A \subseteq A$ to relate the history and future
regexes of two \tyName{}s under the current type context $\Gamma$, as
well as the return type using the standard subtyping relation on pure
refinement types. Notably, this subtyping relation is covariant in the
history regex and contravariant in the future regex, yielding behavior
similar to the reverse rule of consequence found in Incorrectness
Logic (IL)~\cite{OHP19}.  Informally, the rule allows specialization of the
history trace (i.e., $H_1 \subseteq H_2$) and generalization of the
future trace (i.e., $F_2 \subseteq F_1$).  In other words, we can
always replace a \tyName{} triple with one that can justify more
behaviors (aka future traces) with fewer witnesses (aka history
traces).  Since the future automata in a \tyName{} can refer to the
pure value it returns, the inclusion check between the future automata
extends the typing context with a binding for the return value.
...
\textsc{TOpHis} can then freely choose a subset of the reachable
...
rcords the effect as an event in the future trace along with any
additional ghost events ($F$).  The \textsc{TReqOp} and the
...
the same id.  The \textsc{TLet} rule requires the binding expression
$e_1$ to generate the first part of its future trace $F_1$, with the
rest of trace produced by the body expression $e_2$ under a new
history that includes $F_1$.

\begin{example}[Operator Application Typing]
  We present a typing derivation for a simple generator that generates $\eff{push}$ and $\eff{pop}$ operations using their signatures given in
  Example~\ref{ex:stack-example}. Consider how we might type check the following expression:
  \vspace{-.4cm} \par\nobreak {\small %
    \begin{align*}
      &\emptyset \vdash \zlet{x}{\Code{pos\_gen\;()}}{\eff{push}\;x; \eff{pop}} : 
  ...
  trace in which the stack is empty (i.e., ghost events
  $\effGray{pushI}$ and $\effGray{popI}$ are recorded with a $0$ index
  and dummy value $0$).  The body of the generator pushes a postive
  number $y$ using the generator \Code{pos\_gen} whose type is
  $\urt{\Int}{0 < \vnu}$; this underapproximate refinement type
  signifies that \Code{pos\_gen} is guaranteed to produce every
  positive integer.  the generator then is expected to pop the
  same value.   We can thus type check its two effectful operation
  applications using the following judgement:
  \vspace{-.4cm} \par\nobreak {\small %
    \begin{flalign*}
    ...
    \end{flalign*}}%
  \noindent
  Using the \textsc{TEffOp} rule to type the first $\eff{push}$ effect,
  we retrieve the operator's types from the operator context $\Delta$:
  \vspace{-.4cm} \par\nobreak {\small %
    \begin{flalign*}
    ...
    \end{flalign*}}\noindent
  We can then apply the subtyping rule (\textsc{SubG}) to allow us to instantiate the
  ghost variable (e.g., $n \mapsto 0$ and $y \mapsto 0$). 
  The rule \textsc{TOpHis} can then be used to specialize a reachable history trace
  ($\msgAGray{pushI}{0\;0}\msgAGray{popI}{0\;0}$) to align with the
  actual calling context. Now, the future regex
  $\msgA{push}{x}\msgAGray{pushI}{1\;x}$ is exactly aligned with the
  ...
  \vspace{-.4cm} \par\nobreak {\small %
    ...
    \end{align*}}%
  \noindent
  As before, we can instantiate the ghost variable $n$ as $0$ and $m$ as $0$ via rule \textsc{SubG}, align the return type via rule \textsc{SubHF} and refine the history regex via the rule \textsc{TOpHis} as shown below:
  \vspace{-.4cm} \par\nobreak {\footnotesize %
    \begin{flalign*}
...
Finally, the nondeterministic choice operator is typed using the
\textsc{TChoice} rule which merges the \tyName{}s of the two branches.
The typing rule for function (\textsc{TFun}) is standard, and the type
rule for recursive functions is modeled after the corresponding IL
rule, unrolling the recursion as needed.  The \textsc{FixBase} rule
types any recursive function with a \tyName{} lacking reachability
constraints (i.e., $\uhat{H}{\urt{b}{\bot}}{\emptyset}$). In contrast,
the \textsc{FixInd} rule permits typing a recursive function with type
...
Although ghost events are constrained by \tyName{}s, they do not
appear in the actual traces produced by operational semantics.  We
define a erasure function $\eraseGhost{\alpha}$ to erase ghost events
in a \tyName{} from the trace $\alpha$.

\paragraph{Type denotations} Similar to other refinement type systems~\cite{JV21},
types in $\DSL$ are denoted as their inhabitants (i.e., $\denot{t}$
and $\denot{\tau}$).  The type context $\Gamma$
is denoted as \emph{substitution} $\sigma$ (i.e., $[x_1\mapsto v_1,
  x_2\mapsto v_2]$) that provides the assignments for binding
variables in $\Gamma$ and the denotation (the denoted language) of
SREs is a set of traces. Then, trace inclusion under a type context
is defined as $\Gamma \vdash A \subseteq A' \doteq \forall \sigma \in
\denot{\Gamma}. \denot{\sigma(A)} \subseteq \denot{\sigma(B)}$.  The
type denotation of $\uhat{H}{x{:}\urt{b}{\phi}}{F}$ is defined as
follows:
% {\small\begin{align*}
...
{\small\begin{align*}
  ...
  % \\[-0.4em]& \quad 
  \{e ~|~ \emptyset \basicvdash e : s \land \forall v{:}b. \phi[v\mapsto v] \impl \forall \alpha_f \in \denot{F[x\mapsto v]}. \exists \alpha_h \in\denot{H[x\mapsto v]}. \msteptr{\eraseGhost{\alpha_h}}{e}{\eraseGhost{\alpha_f}}{v} \}
\end{align*}}\noindent
...
compositional type rules. This formulation additionally avoids
...
\begin{corollary}[Type Soundness]\label{theorem:sound} A generator \(e\) that satisfies \(\emptyset \vdash e : \uhat{\epsilon}{\Unit}{F}\) must produce all traces accepted by $F$, i.e., $\forall \alpha \in \denot{F}. \msteptr{[]}{e}{\eraseGhost{\alpha}}{()}$.
\end{corollary}
