...
{\small
  \begin{alignat*}{2}
    ...
    \text{\textbf{Pure Operations}}& \quad & \primop ::= \quad & {+} ~|~ {-} ~|~ {==} ~|~ {<} ~|~ {\leq} ~|~ \Code{not} ~|~ \Code{Random}(b) ~|~ ...\\
    ...
    \\ \text{\textbf{Values }}& \quad  & v ::= \quad & v ~|~ x
    \\ \text{\textbf{Expressions}}& \quad & e ::=\quad & v
    % ~|~ \zrand{x}{e}
    ~|~ \zlet{x}{\primop\ \overline{v}}{e}
    \\ &\quad&\quad& ~|~ \zgen{op}{\overline{v}}{e} ~|~ \zobs{\overline{x}}{op}{e}
    \\ &\quad&\quad& ~|~ e;e ~|~ e \oplus e ~|~ \zassume{v}{e}
    % ~|~ \zloop{e}
...
\begin{alignat*}{3}
    ...
    \\ \zif{v}{e_1}{e_2} &\doteq (\zassume{v}{e_1}) \oplus 
...
\end{prooftree}
    ...
    % \end{prooftree}
    \begin{prooftree}
      \hypo{ \src{\zevent{op}{v}} = \envI \quad \alpha, \beta \vDash \zevent{op}{\overline{v}} \Downarrow \beta'}
      \infer1[\textsc{\small StGen}]{
        \steptr{\alpha}{(\buf', \zgen{op}{\overline{v}}{e})}{[\zevent{op}{\overline{v}}]}{(\buf', e)}
      }
    \end{prooftree}
    \\ \ \\ \ \\    
    \begin{prooftree}
    \mhypo{70mm}
    {$\buf = [\zevent{op}{\overline{v}}] \listconcat \buf'$ \quad
    $\alpha, \buf' \vDash \zevent{op}{\overline{v}} \Downarrow \beta''$ \quad
    % $\select{\buf, \zevent{op}{\overline{v}}, \buf'} $ \quad
     $e' = e[\overline{x \mapsto v}]$
     }
      \infer1[\textsc{\small StObserve}]{
        \steptr{\alpha}{(\buf, \zobs{\overline{x}}{op}{e})}{[\zevent{op}{\overline{v}}]}{(\beta'', e')}
      }
    \end{prooftree}
    \\ \ \\ \ \\
...
\\ \text{\textbf{Qualifiers}}& \quad & \phi ::= \quad & v ~|~ \bot ~|~ \top  ~|~ \neg \phi ~|~ \phi \land \phi ~|~ \phi \lor \phi ~|~ \phi \impl \phi ~|~ \forall x{:}b.\, \phi
    ...
    \\ \text{\textbf{Refinement Base Types}}& \quad & t  ::= \quad & \rawnuot{b}{\phi} ~|~ \rawnuut{b}{\phi}
...
\quad
\mprooftr{28mm}
{$\Gamma,  x{:}\rawnuut{b} \vdash \tau \termsyn e$}
{TGhost}
{$\Gamma \vdash x{:}\rawnuut{b}{\phi}\garr \tau \termsyn e$}
\quad
\mprooftr{35mm}
{ $\Gamma, x{:}\rawnuut{b}{\phi} \vdash \tau \termsyn e$}
{TVar}
...
{\footnotesize\begin{align*}
    ...
    \\&\tau = x\hasuty{int}{\top}\garr y\hasuty{int}{\vnu \not = x}\garr \exArw
\end{align*}}\noindent
Now we convert it into the following synthesis judgement
\begin{align*}
    \emptyset \vdash x\hasuty{int}{\top}\garr y\hasuty{int}{\vnu \not = x}\garr \rg{\epsilon}{\exArw}{\anyA^*} \termsyn e
\end{align*}\noindent
We apply subsumption rule to specialize it:

{\footnotesize\mprooftr{90mm}
{$\Gamma \vdash x\hasuty{int}{\vnu \not= 0}\garr x\hasuty{int}{\vnu =  0}\garr \rg{\epsilon}{\exArw}{\anyA^*} \termsyn e$ \quad 
 $\Gamma \vdash x\hasuty{int}{\vnu \not= 0}\garr x\hasuty{int}{\vnu =  0}\garr \rg{\epsilon}{\exArw}{\anyA^*} <: x\hasuty{int}{\top}\garr y\hasuty{int}{\vnu \not = x}\garr \rg{\epsilon}{\exArw}{\anyA^*}$}
{TSub}
{$\Gamma \vdash x\hasuty{int}{\top}\garr y\hasuty{int}{\vnu \not = x}\garr \rg{\epsilon}{\exArw}{\anyA^*} \termsyn e$}}
\\ \ 
...
{$\Gamma \vdash x\hasuty{int}{\vnu \not= 0}\garr \rg{\epsilon}{\exArw[y \mapsto 0]}{\anyA^*} \termsyn e$ \quad 
 $\Gamma \vdash x\hasuty{int}{\vnu \not= 0}\garr \rg{\epsilon}{\exArw[y \mapsto 0]}{\anyA^*} <: x\hasuty{int}{\top}\garr y\hasuty{int}{\vnu \not = x}\garr \rg{\epsilon}{\exArw}{\anyA^*}$}
{TSub}
{$\Gamma \vdash x\hasuty{int}{\top}\garr y\hasuty{int}{\vnu \not = x}\garr \rg{\epsilon}{\exArw}{\anyA^*} \termsyn e$}}
\\ \ 
...
\begin{align*}
  ...
  \\ &\denot{\rg{H}{A}{F}} &&\doteq \{e ~|~ \emptyset \basicvdash e : \eraserf{t} \land \forall e_H\; \alpha_H\; e_F\; \alpha_F\; v\; \beta_H\;\beta. \alpha_H \in \denot{H} \impl
  \\&&& \qquad\qquad [] \vDash ([], e_H;e;e_F) \mharr{\alpha_H} (\beta_H, e;e_F) \mharr{\alpha} (\beta, e_F) \mharr{\alpha_F} ([], v) \impl \alpha\in\denot{A} \land \alpha_F\in\denot{F} \}
  %  \\ &\denot{\hfive{A}{B}{t}{B'}{A'}} &&\doteq \{e ~|~ \emptyset \basicvdash e : \eraserf{t} \land \forall \alpha\in\denot{A}\; \beta\in\denot{B}\;\alpha'\, \beta'\,v. 
