...
into a set of abstract traces. The second, $\termDerive$, generates a
straightline program from a single refined abstract trace. The third,
$\S{synRecursion}$, generates a recursive program using a recursion
template. \BD{We need to double check that the names of the three
  synthesis functions are consistent with the main text. }
...
\begin{algorithm}[t!]
  ...
  \Match{$A$}{
    ...
    \Case{$A^*$}{
      \Match{$\normPlan(A)$}{
        \lCase{$\emptyset$}{ \Return{$\emptyset$} }
        \lCase{$\bigcup \msg{op}{\phi}$}{ \Return{$\{[\overline{\msg{op}{\phi}^\bot}])^*\}$} }
...
\autoref{algo:norm}, recursively translates an input SRE into a set of
...
unchanged. \BD{We should add a minimization step to
  $\normPlan{}$. What does the very last case (line 12) corresponds to
  in the current grammar? The main text says that TermDerive filters
  out ghost events and variables, but it does not look like that is
  the case. Which version is right?}.
...
\begin{algorithm}[t!]
  ...
  \BD{Why do we add asserts over return values instead of assumes?
    \tyName{}s underapproximations, so it's a bit odd to flag
    inconsistent behaviors as errors ...}
