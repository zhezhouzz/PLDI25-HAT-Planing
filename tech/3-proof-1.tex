...
In the proof of this paper, we omit the proof of basic typing $\basicvdash e : s$, where all terms and qualifiers in our typing rules and theorems are basic type safe.
...
\subsection{Lemmas for operational semantics}

\begin{lemma}\label{lemma:op-gen}[One step reduction on Gen expression under context] For all type context $\Gamma$, and term $\zgen{op}{\overline{v}}{e}$, trace $\alpha$ and $\alpha_h$, buffer $\beta$ and $\beta'$ that are well-formed under $\Gamma$, we have:
{\small\begin{align*}
    \forall \sigma, \sigma \in \denot{\Gamma}.
    &\sigma(\alpha_h) \models \sigma(\zevent{op}{\overline{v}}) \Downarrow \sigma(\beta_{\eff{op}}) \impl
    \\&\msteptr{\sigma(\alpha_h;\zevent{op}{\overline{v}})}{(\sigma(\beta \cup \beta_{\eff{op}}, e))}{\sigma(\alpha)}{(\sigma(\beta'), \sigma(e'))} \impl
    \\&\msteptr{\sigma(\alpha_h)}{(\sigma(\beta), \sigma(\zgen{op}{\overline{v}}{e}))}{\sigma(\alpha)}{(\sigma(\beta'), \sigma(e'))}
\end{align*}}
\end{lemma}

\subsection{Lemma for denotations}
...
% \\& \alpha_h \listconcat [\zevent{op}{\overline{\sigma(v_i)}}] \in \denot{\sigma(H \seqA \lastA\msg{op}{\phi\msubst{x_i}{v_i}})}

\subsection{Lemma for Subtyping}
...
\begin{lemma}\label{lemma:subt}Substitution Lemma] For Given type context $\Gamma$, variable $x$, well-formed pure refinement type $t$,\PAT $\tau$ and term $e$:
  $\Gamma, x{:}t;\Delta;\Theta \vdash e : \tau \impl \forall v. \Gamma \vdash v : t \impl \Gamma;\Delta;\Theta \vdash e[x\mapsto v] : \tau[x\mapsto v]$
\end{lemma}

\subsection{Lemma for handler context}

\begin{definition}[Well-formed handler context]
  \label{lemma:built-in-typing-ap}
  The handler specification $\Delta$ is well-formed iff for all operator $\eff{op}$ and its \PAT $\overline{y{:}b} \garr \overline{x{:}t} \sarr \rg{H}{\lastA\msgB{op}{\overline{y}}{\phi}}{F}$ and capability $\Theta$ in $\Delta$ satisfying
{\small\begin{align*}
    &\forall \overline{y{:}b}.\forall \alpha_h \in \denot{H}. \forall \overline{c \in \denotation{t}}. \forall \alpha_f \in \denot{F}. \phi\msubst{y}{c} \impl
    \\&\quad \exists \overline{\alpha_i}. \exists \overline{m_i}.\; \alpha_1 \listconcat [m_1] \listconcat ... [m_{n}] \listconcat \alpha_{n+1} = \alpha_f \land \alpha_h \vDash \zevent{op}{\overline{c}} \Downarrow \{m_i\} \land \forall \eff{op_i}. \eff{op_i} \in \Theta \iff \exists \overline{c_i}.m_i = \zevent{op_i}{\overline{c_i}}
\end{align*}}
\end{definition}
...
{\small\begin{align*}
    ...
    \exists \overline{\alpha_i}. \exists \overline{m_i}.\; \alpha_1 \listconcat [m_1] \listconcat ... [m_{n}] \listconcat \alpha_{n+1} = \alpha_f \land
    \\&\quad \alpha_h \vDash \zevent{op}{\overline{c}} \Downarrow \{m_i\} \land \sigma(\phi)\msubst{y}{c} \land  \forall \eff{op_i}. \eff{op_i} \in \Theta {\iff} \exists \overline{c_i}.m_i = \zevent{op_i}{\overline{c_i}}
\end{align*}}
\end{lemma}
...
$\forall i. \Gamma \vdash v_i : t_i$ \quad $\Gamma \vdash \nuot{unit}{\top} <: \nuot{unit}{\phi\msubst{x_i}{v_i}}$ \quad
...
&\forall \sigma \in \denot{\Gamma}. \sigma(\phi)\msubst{x_i}{v_i} \ptag{\ref{found:1-4} and Lemma~\ref{theorem:pure-fundamental-ap}} \label{found:1-8}
\end{alignat}}\noindent
...
\\& \forall \sigma\in\denot{\Gamma}. \forall c_x. \primop(\overline{\sigma(v)}) \Downarrow c_x \impl \sigma(e[x\mapsto c_x]) \in \denot{\sigma(\tau)} \ptag{\ref{found:3-19} and $\tau$ is well-formed under $\Gamma$} \label{found:3-10}
\end{alignat}}\noindent
...
% According to well-formed handler context Lemma~\ref{lemma:built-in-typing-sub}, as well as assumption \ref{found:1-1} and \ref{found:1-2}, we have
% {\footnotesize\begin{alignat}{2}
% &\forall \sigma \in \denot{\Gamma}. \forall \alpha_h \in \denot{\sigma(H)}. \forall \overline{c \in \denotation{\sigma(t)}}. \forall \alpha_f \in \denot{\sigma(A \seqA F)}. \sigma(\phi)\msubst{y}{c} \impl \exists \overline{\alpha_i}. \exists \overline{m_i}.\; \notag 
% \\&\quad \alpha_1 \listconcat [m_1] \listconcat ... [m_{n}] \listconcat \alpha_{n+1} = \alpha_f \land \alpha_h \vDash \zevent{op}{\overline{c}} \Downarrow \{m_i\} \land \forall \eff{op_i}. \eff{op_i} \in \Theta' \iff \exists \overline{c_i}.m_i = \zevent{op_i}{\overline{c_i}} \ptag{Lemma~\ref{lemma:built-in-typing-sub}} \label{found:1-9}
% \\&\forall \sigma \in \denot{\Gamma}. \forall \alpha_h \in \denot{\sigma(H)}. \forall \alpha_f \in \denot{\sigma(A \seqA F)}. \exists \overline{\alpha_i}. \exists \overline{m_i}.\; \notag 
% \\&\quad \alpha_1 \listconcat [m_1] \listconcat ... [m_{n}] \listconcat \alpha_{n+1} = \alpha_f \land \alpha_h \vDash \zevent{op}{\overline{c}} \Downarrow \{m_i\} \land \forall \eff{op_i}. \eff{op_i} \in \Theta' \iff \exists \overline{c_i}.m_i = \zevent{op_i}{\overline{c_i}} \ptag{\ref{found:1-9}, \ref{found:1-7}, and \ref{found:1-8}} \label{found:1-10}
% \end{alignat}}\noindent